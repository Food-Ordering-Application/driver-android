        ...
        mapView.getMapAsync(mapBoxMap -> mapBoxMap.setStyle(getString(R.string.navigation_guidance_day), style -> {
            ...
            if (currentOrder != null) {
                ...
                switch (DriverModeSession.getInstance()) {
                    case ON_GOING:
                        visibleDeliveryInfo();
                        destinationPoint= restaurantPoint;
                        addMarkerAndGetRoute(RESTAURANT_SOURCE_ID);
                        break;
                    case PICKED_UP:
                        visibleDeliveryInfo();
                        destinationPoint= customerPoint;
                        addMarkerAndGetRoute(CUSTOMER_SOURCE_ID);
                        break;
                    case COMPLETED:
                    case CANCELLED:
                    default:
...
//        mapView.setLayoutParams(layoutParams2);
    ...
    private void addMarkerAndGetRoute(String sourceId) {
        if (ActivityCompat.checkSelfPermission(getActivity(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(getActivity(), Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            return;
        }else{
            fusedLocationClient.getLastLocation()
